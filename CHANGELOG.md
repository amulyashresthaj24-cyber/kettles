...
Session-timer correctness, then AI attribution in reports.
...
- **Project budget alerts.** `lib/budget.ts` computes lifetime budget health — billable spend at the effective project-or-client rate, what is left, and roughly how many billable hours that buys. Projects at 80% or over now surface as a banner on the dashboard (nothing renders while every budget is healthy), and the project page shows a spend bar with hours remaining.

### Changed

- The report's "Budget Used %" divides *period* earnings by the *lifetime* budget, so a week-long filter made a nearly-spent budget look untouched. The figure is still useful as period spend against budget, so it stays — relabelled "Budget (period)" in the table and "Budget Used % (this period)" in the Excel export. Budget health proper now lives on the project page.
