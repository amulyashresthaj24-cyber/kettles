...
export function EditProjectModal({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setError(null);
    try {
      // Rename linked client / create / clear — never a multi-client picker.
      const clientId = await resolveProjectClientLink({
        linkedClientId,
        clientName,
      });
        ...
        <div className="flex items-center justify-between gap-sm px-xl py-lg border-t border-border-subtle">
          <div className="flex items-center gap-sm flex-wrap">
            ...
            <ClientNameField
              ...
              placeholder={linkedClientId ? "Client name" : "Add client"}
            />
          </div>
